非常好！你的澄清和决策非常清晰，这正是高质量技术讨论所需要的。你对具体场景的考量，比如数据稀疏的品种、Worker 和 PersistenceTask 之间的责任划分，都非常有价值。

我完全理解并同意你的观点，特别是关于数据拉取模型的第二点。让 Worker 提供已更新的数据并负责状态流转，确实比让外部消费者去猜测状态要清晰得多。这个模型中的“契约”更加明确。

现在，我将我们达成的共识和你的澄清，完整地、精确地更新到架构文档中。
...
主动推送快照 (Active Snapshot Push Model): 外部系统（如持久化任务、API服务）通过 WorkerReadHandle 发送数据请求。Worker在收到请求后，主动筛选出已更新的K线数据，形成快照返回给请求方，并管理自身状态的流转。
