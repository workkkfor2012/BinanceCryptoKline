...
$env:KLINE_TEST_MODE = "true"  # 启用测试模式，使用8个主流品种
$env:ENABLE_PERF_LOG = "1"     # 启用性能日志分析

Write-Host "🔥 性能日志分析已启用，将生成 logs\performance.folded" -ForegroundColor Magenta
Write-Host "🧪 测试模式已启用，将订阅8个主流品种 (每个worker分配2个)" -ForegroundColor Cyan

$buildMode = Get-BuildMode
Write-Host "🚀 启动K线聚合服务 - 分区聚合版架构 ($buildMode) [测试模式]" -ForegroundColor Yellow
