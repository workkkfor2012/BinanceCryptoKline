...
impl Worker {
    ...
    pub async fn new(
        ...
        info!(log_type = "low_freq", worker_id, initial_capacity_symbols, total_slots, "分配状态缓冲区");
    ...
    fn process_trade(&mut self, trade: AggTradeData) {
        let local_index = match self.local_symbol_cache.get(&trade.symbol) {
            Some(&idx) => idx,
            None => {
                warn!(worker_id = self.worker_id, symbol = %trade.symbol, "收到未被此Worker管理的交易数据");
                return;
            }
        };
        ...
        if base_offset >= self.kline_states.len() {
            error!(worker_id = self.worker_id, symbol = %trade.symbol, local_index, "计算的本地索引超出预分配内存边界！");
            return;
        }
    ...
    fn process_clock_tick(&mut self, current_time: i64) {
        trace!(worker_id = self.worker_id, time = current_time, "开始处理时钟滴答");
        let num_periods = self.periods.len();
        let write_buffer = if self.active_buffer_index == 0 { &mut self.snapshot_buffers.0 } else { &mut self.snapshot_buffers.1 };
            ...
            for period_idx in 0..num_periods {
                ...
                if kline.is_initialized && !kline.is_final {
                    ...
                    if current_time >= kline.open_time + interval_ms {
                        kline.is_final = true;
                        debug!(worker_id = self.worker_id, local_idx, period_idx, open_time = kline.open_time, "将一根K线标记为 is_final");
    ...
    async fn process_command(&mut self, cmd: WorkerCmd) {
        info!(worker_id = self.worker_id, command = ?cmd, "开始处理管理指令");
        match cmd {
            WorkerCmd::AddSymbol { symbol, global_index, ack } => {
                ...
                if ack.send(Ok(())).is_err() {
                    warn!(worker_id = self.worker_id, %symbol, "向品种管理器发送确认信号失败，上游可能已超时");
                }
            }
        }
    }
...
pub async fn run_io_loop(
    ...
    loop {
        ...
        // 为单次连接会话创建一个性能span
        let session_span = tracing::trace_span!(
            target: "perf",
            "io_connection_session",
            worker_id,
            subscribed_symbols = managed_symbols.len()
        );
        ...
        tokio::select! {
            ...
            result = client.start().instrument(session_span) => {
                if let Err(e) = result {
                    error!(worker_id, error_chain = format!("{:#}", e), "WebSocket客户端失败，准备重连");
                }
                // WebSocket 连接断开，等待重连
                sleep(Duration::from_secs(1)).await;
            },
            Some(cmd) = ws_cmd_rx.recv() => {
                info!(worker_id, ?cmd, "收到订阅指令，更新待订阅列表");
                match cmd {
                    WsCmd::Subscribe(new_symbols) => {
                        // 更新管理的品种列表
                        managed_symbols.extend(new_symbols.clone());
...
pub async fn persistence_task(
    ...
    loop {
        tokio::select! {
            _ = interval.tick() => {
                // [核心] 遵循模式四：创建根Span
                let persistence_span = info_span!(
                    parent: None,
                ...
                perform_persistence_cycle(&db, &worker_handles, &index_to_symbol, &periods)
            ...
            _ = shutdown_rx.changed() => {
                if *shutdown_rx.borrow() {
                    ...
                    let final_persistence_span = info_span!(
                        parent: None,
                        "final_persistence_transaction",
                        tx_id = %format!("persist:final:{}", chrono::Utc::now().timestamp_millis()),
                        log_type = "high_freq"
                    );

                    perform_persistence_cycle(&db, &worker_handles, &index_to_symbol, &periods)
                        .instrument(final_persistence_span)
                        .await;
                    break;
                }
            }
...
async fn perform_persistence_cycle(
    ...
    for (worker_id, result) in results {
        if let Ok(snapshot) = result {
            ...
            // [采纳另一个AI的优点]
            error!(worker_id, error_chain = format!("{:#}", e), "从Worker获取快照失败");
        }
    }
    ...
    if !klines_to_save.is_empty() {
        ...
        if let Err(e) = db.upsert_klines_batch(klines_to_save) {
            error!(error_chain = format!("{:#}", e), "保存K线数据到数据库失败");
        } else {
            // [坚持我的观点] 这是归属于 high_freq 事务的普通 Info 事件。
            info!(kline_count = count, "成功保存{}条K线数据", count);
        }
    } else {
