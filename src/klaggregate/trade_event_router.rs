...
impl TradeEventRouter {
    ...
    pub fn new() -> Self {
        info!(target: "trade_event_router", "创建交易事件路由器");
    ...
    pub async fn register_aggregator(
        ...
        debug!(target: "trade_event_router", "注册品种聚合器: symbol={}", symbol);

        let mut aggregators = self.aggregators.write().await;

        if aggregators.contains_key(&symbol) {
            warn!(target: "trade_event_router", "品种聚合器已存在，将被替换: symbol={}", symbol);
        }
        ...
        debug!(target: "trade_event_router", "品种聚合器注册完成: symbol={}", symbol);
        Ok(())
    }
    ...
    pub async fn route_trade_event(&self, trade: AggTradeData) -> Result<()> {
        ...
        if let Some(aggregator) = aggregators.get(&trade.symbol) {
            ...
            match aggregator.process_agg_trade(&trade).await {
                ...
                Err(e) => {
                    error!(target: "trade_event_router", "处理品种交易失败: symbol={}, error={}", trade.symbol, e);
            ...
            warn!(target: "trade_event_router", "未找到品种聚合器: symbol={}", trade.symbol);
    ...
    pub async fn finalize_all_aggregators(&self) -> Result<()> {
        info!(target: "trade_event_router", "强制完成所有聚合器的K线");
        ...
        for (symbol, aggregator) in aggregators.iter() {
            match aggregator.finalize_all_klines().await {
                Ok(()) => {
                    success_count += 1;
                    debug!(target: "trade_event_router", "品种K线强制完成成功: symbol={}", symbol);
                }
                Err(e) => {
                    error_count += 1;
                    error!(target: "trade_event_router", "品种K线强制完成失败: symbol={}, error={}", symbol, e);
                }
            }
        }

        info!(target: "trade_event_router", "强制完成K线操作完成: success_count={}, error_count={}", success_count, error_count);
    ...
    pub async fn reset_statistics(&self) {
        ...
        *error_count = 0;
        ...
        info!(target: "trade_event_router", "路由统计信息已重置");
    }
