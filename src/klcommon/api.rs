...
impl BinanceApi {
    ...
    pub async fn download_klines(&self, task: &DownloadTask) -> Result<Vec<Kline>> {
        ...
        let response = match request.send().await {
            ...
            Err(e) => {
                // 只在错误时记录请求URL
                error!("{}/{}: 请求失败: URL={}, 错误: {}", task.symbol, task.interval, fapi_url, e);
                return Err(e.into());
            }
        };
        ...
        if !response.status().is_success() {
            ...
            let text = response.text().await?;
            // 只在失败时打印URL
            error!("从fapi下载 {} 的K线失败: {} - {}, URL: {}", task.symbol, status, text, fapi_url);
            return Err(AppError::ApiError(format!(
                "从fapi下载 {} 的K线失败: {} - {}",
        ...
        let raw_klines: Vec<Vec<Value>> = match serde_json::from_str(&response_text) {
            Ok(data) => data,
            Err(e) => {
                error!("{}/{}: 解析JSON失败: {}, 原始响应: {}", task.symbol, task.interval, e, response_text);
                return Err(AppError::JsonError(e));
            }
        };
        ...
        if raw_klines.is_empty() {
            error!("{}/{}: 返回空结果，原始响应: {}", task.symbol, task.interval, response_text);
    ...
    pub async fn download_continuous_klines(&self, task: &DownloadTask) -> Result<Vec<Kline>> {
        ...
        let response = match request.send().await {
            ...
            Err(e) => {
                // 只在错误时记录请求URL
                error!("{}/{}: 连续合约请求失败: URL={}, 错误: {}", task.symbol, task.interval, fapi_url, e);
                return Err(e.into());
            }
        };
        ...
        let raw_klines: Vec<Vec<Value>> = match serde_json::from_str(&response_text) {
            Ok(data) => data,
            Err(e) => {
                error!("{}/{}: 连续合约解析JSON失败: {}, 原始响应: {}", task.symbol, task.interval, e, response_text);
                return Err(AppError::JsonError(e));
            }
        };
        ...
        // 检查是否为空结果
        if raw_klines.is_empty() {
            error!("{}/{}: 连续合约返回空结果，原始响应: {}", task.symbol, task.interval, response_text);
            return Err(AppError::DataError(format!(
                "连续合约空结果，原始响应: {}",
                response_text
            )));
