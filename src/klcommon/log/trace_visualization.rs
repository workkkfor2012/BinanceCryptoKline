...
/// 辅助函数：提取 span 的 trace_id 和 parent_id
/// 遵循 DRY 原则，避免在 on_new_span 和 on_close 中重复代码
fn get_span_identity<S>(span: &tracing_subscriber::registry::SpanRef<S>) -> (String, Option<String>)
where
    S: Subscriber + for<'lookup> tracing_subscriber::registry::LookupSpan<'lookup>,
{
    // 提取 trace_id：递归向上查找根 span
    fn find_root_id<S2: for<'a> tracing_subscriber::registry::LookupSpan<'a>>(
        s: &tracing_subscriber::registry::SpanRef<S2>
    ) -> Id {
        s.parent().map_or_else(|| s.id(), |p| find_root_id(&p))
    }
    ...
    S: Subscriber + for<'lookup> tracing_subscriber::registry::LookupSpan<'lookup>,
{
    ...
    fn on_new_span(&self, _attrs: &tracing::span::Attributes<'_>, id: &Id, ctx: Context<'_, S>) {
        ...
        // 使用辅助函数提取 trace_id 和 parent_id
        let (trace_id, parent_id) = get_span_identity(&span);
    ...
    fn on_close(&self, id: Id, ctx: Context<'_, S>) {
        ...
        // 🚨 关键修复：使用辅助函数提取 trace_id 和 parent_id
        // 这解决了 span_end 事件中 parent_id 为 null 的严重问题
        let (trace_id, parent_id) = get_span_identity(&span);
        ...
        let span_event = serde_json::json!({
            ...
            "fields": {
                ...
                "trace_id": trace_id,
                "span_id": format!("span_{}", id.into_u64()),
                "parent_id": parent_id, // 🎯 关键修复：添加缺失的 parent_id
                "span_name": metadata.name(),
