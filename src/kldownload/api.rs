...
use super::models::{DownloadTask, ExchangeInfo, Kline};
...
impl BinanceApi {
    ...
    pub async fn get_exchange_info(&self) -> Result<ExchangeInfo> {
        ...
        // 手动构建 ExchangeInfo 对象
        let mut exchange_info = ExchangeInfo {
            timezone: json_value["timezone"].as_str().unwrap_or("").to_string(),
            server_time: json_value["serverTime"].as_i64().unwrap_or(0),
            rate_limits: Vec::new(),
        ...
        // 解析交易对
        if let Some(symbols_array) = json_value["symbols"].as_array() {
            for symbol_value in symbols_array {
                let symbol = super::models::Symbol {
                    symbol: symbol_value["symbol"].as_str().unwrap_or("").to_string(),
                    status: symbol_value["status"].as_str().unwrap_or("").to_string(),
                    base_asset: symbol_value["baseAsset"].as_str().unwrap_or("").to_string(),
                    quote_asset: symbol_value["quoteAsset"].as_str().unwrap_or("").to_string(),
                    margin_asset: symbol_value["marginAsset"].as_str().unwrap_or("").to_string(),
                    price_precision: symbol_value["pricePrecision"].as_i64().unwrap_or(0) as i32,
                    quantity_precision: symbol_value["quantityPrecision"].as_i64().unwrap_or(0) as i32,
                    base_asset_precision: symbol_value["baseAssetPrecision"].as_i64().unwrap_or(0) as i32,
                    quote_precision: symbol_value["quotePrecision"].as_i64().unwrap_or(0) as i32,
                    underlying_type: symbol_value["underlyingType"].as_str().unwrap_or("").to_string(),
                    underlying_sub_type: Vec::new(),
                    settle_plan: symbol_value["settlePlan"].as_i64().unwrap_or(0) as i32,
                    trigger_protect: symbol_value["triggerProtect"].as_str().unwrap_or("").to_string(),
                    filters: Vec::new(),
                    order_type: Vec::new(),
                    time_in_force: Vec::new(),
                    liquidation_fee: symbol_value["liquidationFee"].as_str().unwrap_or("").to_string(),
                    market_take_bound: symbol_value["marketTakeBound"].as_str().unwrap_or("").to_string(),
                    extra: std::collections::HashMap::new(),
                };
                exchange_info.symbols.push(symbol);
            }
        }
